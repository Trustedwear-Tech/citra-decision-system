...
| 2 | Two aggregation endpoints (org card, app drill-down) | ✅ BUILT 2026-07-19 — `GET /org/screening-stats?period=week\|month\|all` (admin-role-gated; totals + per-app rows + display names) and `GET /apps/{slug}/screening-stats` (same gate as fraud-calibration; signals w/ plain labels, verbatim dismissal reasons, §3 advisories at ≥5 verdicts & ≥70% dismissal). `fraud_synthesis.screening_stats` + static `SIGNAL_ADVISORIES` map (advisory text per signal key — the UI renders these verbatim). Latest-row-per-record dedupe; false-alarm rate = dismissed ÷ judged. 10 unit tests + live smoke on local (org endpoint 200/403/422; drill-down shape verified against seeded the demo tenant rows) |
