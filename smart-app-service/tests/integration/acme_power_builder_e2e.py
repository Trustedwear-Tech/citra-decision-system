...
SCRIPTED_REPLIES: list[tuple[str, str]] = [
    ("no registered dept-mcps",
     "The acme-power dept MCP is registered - re-run discovery. If it truly is missing, proceed with a placeholder on field_operations.complaints."),
    ("no data connections",
     "Use the Revenue Protection & Field Operations source - field_operations.complaints."),
    ("placeholder",
     "Yes, proceed with a placeholder source if discovery is empty."),
    ("clarifying questions",
     "Goal: triage inbound complaints. Users: care-line officers and their supervisor. "
     "The AI recommends routing (assigned officer, priority, SLA); an officer approves or corrects every one. "
     "Theft reports must go to revenue protection."),
    ("sample input", "Yes, those decisions look right. Approve."),
    ("decision table", "Looks good, ship it."),
    ("multi-page",                 "Multi-page, please."),
    ("which fits how your users",  "Multi-page."),
    ("landing page",               "Inbox of new complaints as the landing page."),
    ("submission form would be",   "No submission form - complaints arrive from the care line."),
    ("queue page",                 "Inbox shares the page with a complaints-by-category chart."),
    ("clicking a row",             "Separate page for the complaint detail."),
    ...
    ("after submitting",           "Back to the inbox."),
    ("queue-and-resolve",          "Queue-and-resolve in the app - no email notification."),
    ("reason code", "Your proposed reason codes look right - keep them."),
    ("facet", "Your facet choices look right - keep them."),
    ("learn from", "Yes - the app should learn from officer corrections."),
...
BA_GOAL = (
    "Build me a customer complaint triage app on the complaints dataset. The "
    "AI should read each new complaint, recommend the routing (assigned "
    "officer, priority, SLA), and an officer approves or corrects every "
    "recommendation before it is applied. Theft reports are the sensitive "
    "case - they must go to revenue protection."
)
...
def mint_jwt() -> str:
    ...
    user_id = "demo.org.admin@acme-power.citra.ai"
    tenant_id = "acme-power"
    ...
    payload = {
        "user_id": user_id,
        "email":   user_id,
        "name":    "Alice Sutton (Org Admin)",
        "org_id":  "acme-power",
        "tenant_id": tenant_id,
        "dept_ids": ["distribution_ops", "vigilance_field", "billing_revenue"],
        "roles":   ["org_admin"],
...
async def start_build(client: httpx.AsyncClient, token: str) -> dict[str, Any]:
    print(f"\n=== POST /build with acme-power complaint-triage goal ===")
    r = await client.post(
...
async def main(args: argparse.Namespace) -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # -- THE TEST: did the builder author a valid case_signature? -----
        verdict = await verify_case_signature(client, token, session)
        session.runtime_findings["case_signature_verdict"] = verdict

    write_report(session)
    print("\n=== CASE-SIGNATURE VERDICT ===")
    print(json.dumps(session.runtime_findings.get("case_signature_verdict"), indent=1))
...
# ---------------------------------------------------------------------------
# case_signature verification - the point of this harness
# ---------------------------------------------------------------------------

async def verify_case_signature(client, token, session) -> dict:
    """Validate the built app's case_signature through the REAL publish gates."""
    import sys as _sys
    ...
    if session.publish_slug:
        try:
            r = await client.get(f"{SMART_APP_URL}/apps/{session.publish_slug}",
                                 headers=auth_headers(token), timeout=15.0)
            if r.status_code == 200:
                body = r.json() or {}
                spec = body.get("app_spec") or body
                source = f"published:{session.publish_slug}"
        except Exception as e:
            print(f"[verify] fetch published app failed: {e}")
    if spec is None:
        for label in reversed(list(session.workspace_snapshots)):
            snap = session.workspace_snapshots[label]
            if "app_spec.json" in snap:
                try:
                    spec = json.loads(snap["app_spec.json"])
                    source = f"workspace:{label}"
                    break
                except Exception:
                    continue
    if spec is None:
        return {"result": "FAIL", "reason": "no app_spec produced at all"}

    sig = spec.get("case_signature")
    if not sig:
        return {"result": "FAIL", "source": source,
                "reason": ("builder did NOT author a case_signature - "
                           "the skill directive did not fire")}

    try:
        from models import AppSpec
        from publish_validators import validate_case_signature, _facet_cardinality
        app = AppSpec.model_validate({**spec, "slug": spec.get("slug") or "e2e-x",
                                      "title": spec.get("title") or "T"})
        errs = validate_case_signature(app)
        cells = 1
        for f in (app.case_signature.facets if app.case_signature else []):
            cells *= max(1, _facet_cardinality(f))
        return {
            "result": "PASS" if not errs else "FAIL",
            "source": source,
            "families": [f.family for f in app.case_signature.facets],
            "reason_codes": [rc.code for rc in app.case_signature.reason_codes],
            "cells": cells,
            "cs01_errors": errs,
        }
    except Exception as e:
        return {"result": "FAIL", "source": source,
                "reason": f"case_signature present but invalid: {e}", "raw": sig}
